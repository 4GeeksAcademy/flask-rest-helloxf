...
@app.route('/users/<int:user_id>/planetafavoritos', methods=['POST'])
def handle_post_planetafavoritos(user_id):
    user = User.query.get(user_id)
    if user is None:
        return jsonify({"message": "El usuario no existe"}), 404
    
    request_body_planeta = request.get_json()
    new_planetafavorito= Planeta_favoritos(usuario_id = user_id,people_id = request_body_planeta["id"])

    db.session.add(new_planetafavorito)
    db.session.commit()
